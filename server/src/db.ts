    ...
    if (vitals.mewsScore >= this.data.settings.alertThresholdMews && vitals.triageLevel === 'red') {
      ...
      if (!existingUnresolved) {
        this.data.alerts.unshift({
          ...
          description: `Pulse: ${vitals.heartRate} BPM, Shock Index: ${(vitals.shockIndex ?? (vitals.heartRate / (vitals.systolicBP || 120))).toFixed(2)}, RR: ${vitals.respiratoryRate}/min, BP: ${vitals.systolicBP}/${vitals.diastolicBP} mmHg.`,
          timestamp: new Date().toISOString(),
          resolved: false,
        });
      }
