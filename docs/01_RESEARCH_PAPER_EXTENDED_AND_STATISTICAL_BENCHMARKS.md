...
## Abstract
In-hospital clinical deterioration and delayed emergency triage remain primary global drivers of preventable in-hospital cardiac arrest (IHCA), intensive care unit (ICU) admissions, and fatal septic shock. Traditional vital signs surveillance relies either on cost-prohibitive wired telemetry monitors ($3,000–$10,000 per bed) or intermittent manual nursing rounds conducted once every 4 to 6 hours. In high-volume general wards—particularly across low- and middle-income countries (LMICs) where nurse-to-patient ratios frequently exceed 1:40—this operational latency creates dangerous monitoring "blindspots." 

In this paper, we present **AegisPulse**, an edge-native, zero-hardware contactless physiological monitoring and clinical decision-support platform. AegisPulse leverages commodity RGB consumer optical sensors (laptop and mobile webcams) to capture facial Remote Photoplethysmography (rPPG) signals via the Plane-Orthogonal-to-Skin (POS) algorithm. By extracting pulsatile arterial blood volume changes in the facial micro-capillary bed (maximizing green-spectrum absorption at 500–560 nm), applying temporal zero-phase 4th-order Butterworth bandpass filtering (0.75–3.33 Hz), and performing dynamic first-derivative zero-crossing peak detection, AegisPulse calculates instantaneous Heart Rate (BPM), Heart Rate Variability (HRV via RMSSD), and Respiratory Rate (RR) in under 5 seconds without physical patient contact.

These contactless biometrics are synthesized on-device with laboratory hematology panels (serum lactate, white blood cell count, creatinine, and platelets) inside a real-time Modified Early Warning Score (MEWS) and quick Sequential Organ Failure Assessment (qSOFA) inference engine. In empirical benchmark validation against medical-grade contact pulse oximetry (Contec CMS50D) across diverse Fitzpatrick skin phototypes (I–VI) and ambient illumination levels (150–800 lux), AegisPulse achieved a Mean Absolute Error (MAE) of **2.14 BPM**, a Pearson correlation coefficient of **$r = 0.962$**, and Bland-Altman 95% limits of agreement of **$-3.8\text{ to }+4.1\text{ BPM}$**, while maintaining client-side CPU consumption below **8%** on standard consumer hardware. AegisPulse demonstrates that pure software-defined biomedical signal processing can eliminate the financial and logistical barriers to continuous in-hospital vital signs surveillance.

**Keywords:** Remote Photoplethysmography (rPPG), Contactless Bio-Sensing, Plane-Orthogonal-to-Skin (POS), Modified Early Warning Score (MEWS), Sepsis-3 (qSOFA), Biomedical Signal Processing, Edge Computing, Health Equity.
...
By delivering continuous hemodynamic monitoring without sensor hardware, AegisPulse offers an immediate solution to the 4-hour nursing blindspot in general wards. In busy district hospitals across India where staffing constraints preclude ICU-level monitoring for post-operative recovery, step-down wards, and infectious isolation facilities, AegisPulse provides a zero-marginal-cost safety net that catches clinical deterioration before irreversible cardiac arrest occurs.

### 7.2 Current Limitations & Mitigation Strategies
1. **Severe Patient Rigors & Tremors**: In patients experiencing acute epileptic seizures or high-grade fever rigors, motion displacement exceeds optical flow stabilization thresholds. AegisPulse mitigates this by flagging a **Low Signal Quality Index ($<50\%$)**, instructing nurses to switch to secondary contact confirmation.
...
3. **Cuffless Blood Pressure Estimation**: While Heart Rate, HRV, and Respiratory Rate are directly measured via optical rPPG, continuous Systolic and Diastolic Blood Pressure currently rely on baseline clinical entry coupled with pulse rate escalation models. Direct optical cuffless BP estimation via Pulse Transit Time (PTT) represents an active engineering priority.

### 7.3 Engineering & Clinical Roadmap
- **Phase I (Current)**: Webcam POS rPPG, MEWS/qSOFA scoring, lab diagnostics fusion, and automated SBAR generation.
- **Phase II (Q4 2026)**: Micro-saccadic eye-tracking integration to automate Glasgow Coma Scale (GCS) and cranial nerve response scoring.
- **Phase III (Q2 2027)**: Multi-bed wide-angle ceiling camera surveillance leveraging WebGPU and Wasm to track up to 6 patient beds simultaneously from a single elevated optical sensor.

---

## 8. Conclusion
AegisPulse demonstrates that advanced physiological surveillance does not necessitate expensive, intrusive physical hardware. By harnessing optical rPPG, digital filtering, and deterministic MEWS/qSOFA risk matrices within an edge-native web runtime, AegisPulse bridges the gap between manual nursing rounds and intensive care telemetry. Achieving an empirical MAE of $2.14\text{ BPM}$ ($r=0.962$) with zero video storage overhead, AegisPulse represents a scalable, accessible blueprint for the future of continuous bedside healthcare monitoring.
