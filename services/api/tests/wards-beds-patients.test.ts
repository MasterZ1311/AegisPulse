...
describe('Wards, Beds, and Patients API', () => {
  ...
  describe('Wards', () => {
    it('GET /api/v1/wards returns list of configured wards', async () => {
      const res = await request(app).get('/api/v1/wards').set(authHeader);
    ...
    it('GET /api/v1/wards/WARD-A returns ward details with beds', async () => {
      const res = await request(app).get('/api/v1/wards/WARD-A').set(authHeader);
      expect(res.status).toBe(200);
    ...
    it('GET /api/v1/wards/WARD-A/overview returns complete ward census', async () => {
      const res = await request(app).get('/api/v1/wards/WARD-A/overview').set(authHeader);
      expect(res.status).toBe(200);
    ...
    it('returns 404 for nonexistent ward', async () => {
      const res = await request(app).get('/api/v1/wards/NONEXISTENT-WARD').set(authHeader);
  ...
  describe('Beds', () => {
    it('GET /api/v1/beds returns all beds', async () => {
      const res = await request(app).get('/api/v1/beds').set(authHeader);
      expect(res.status).toBe(200);
      expect(res.body.data.length).toBe(6);
    });

    it('GET /api/v1/beds/:bedId returns bed details and occupant', async () => {
      const beds = await request(app).get('/api/v1/beds').set(authHeader);
      const firstBedId = beds.body.data[0].id;

      const res = await request(app).get(`/api/v1/beds/${firstBedId}`).set(authHeader);
      expect(res.status).toBe(200);
    ...
    it('returns 404 for nonexistent bed', async () => {
      const res = await request(app).get('/api/v1/beds/nonexistent-bed-99').set(authHeader);
      expect(res.status).toBe(404);
    });
  });
  ...
  describe('Patients', () => {
    ...
    it('GET /api/v1/patients/:patientId returns patient summary with latest observations', async () => {
      ...
      const res = await request(app).get(`/api/v1/patients/${firstPatientId}`).set(authHeader);
      expect(res.status).toBe(200);
    ...
    it('returns 404 for nonexistent patient', async () => {
      const res = await request(app).get('/api/v1/patients/nonexistent-patient-99').set(authHeader);
      expect(res.status).toBe(404);
    });
  });
});
